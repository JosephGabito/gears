...
class Gears_Portfolio {
    ...
    public function __construct() {
        ...
        $this->post_type_updated_messages( $messages );
    ...
    public function post_type_updated_messages( $messages ) {
    	...
    	$messages['gears-portfolio'] = array(
    		...
    		1  => __( 'Portfolio updated.', 'gears' ),
    		2  => __( 'Custom field updated.', 'gears' ),
    		3  => __( 'Custom field deleted.', 'gears' ),
    		4  => __( 'Portfolio updated.', 'gears' ),
    		/* translators: %s: date and time of the revision */
    		5  => isset( $_GET['revision'] ) ? sprintf( __( 'Portfolio restored to revision from %s', 'gears' ), wp_post_revision_title( (int) $_GET['revision'], false ) ) : false,
    		6  => __( 'Portfolio published.', 'gears' ),
    		7  => __( 'Portfolio saved.', 'gears' ),
    		8  => __( 'Portfolio submitted.', 'gears' ),
    		9  => sprintf(
    			__( 'Portfolio scheduled for: <strong>%1$s</strong>.', 'gears' ),
    			// translators: Publish box date format, see http://php.net/date
    			date_i18n( __( 'M j, Y @ G:i', 'gears' ), strtotime( $post->post_date ) )
    		),
    		10 => __( 'Portfolio draft updated.', 'gears' )
    	);
        ...
        foreach ( $taxonomies as $taxonomy_slug => $taxonomy ) {
            ...
            if ( ! empty( $terms ) ) {
                ...
                foreach ( $terms as $term ) {
                    ...
                    $categories[] = sprintf( '<li><a href="%1$s">%2$s</a></li>',
